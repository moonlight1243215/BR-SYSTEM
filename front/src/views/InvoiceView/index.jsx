...
const MainView = (props) => {
  ...
  const billButton = (id) => {  // change value of bill to "done" after clicking button
    const updatedData = data.map(item => {
      if (item.id === id) {
        return { ...item, bill: item.bill === "progres" ? "done" : item.bill };
      }
      return item;
    });
  ...
  const exampleMap = filteredData.map((e, key) => (
    <div
      className={`${styles.element} ${e.invoice === "done" ? styles.blueBackground : ""} ${e.bill === "done" ? styles.blueFont : ""}`}
      key={key}
    >
